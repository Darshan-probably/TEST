...
import { FileUp, Upload, Download, Settings, Loader, Eye } from 'lucide-react';
...
export default function InvoiceUploadForm() {
  ...
  const handlePreview = async (e) => {
    e.preventDefault();
    if (!file) {
      setErrorMessage('Please select a file to preview');
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.file_path) {
        setOutputDirectory(data.file_path);
      } else {
        // If backend doesn't return a path, construct an example path
        setOutputDirectory(`outputs/${file.name.replace('.xlsx', '_processed.xlsx')}`);
      }
    } catch (error) {
        ...
        <div className="text-center p-6 bg-green-50 rounded-lg mb-6">
          ...
          {outputDirectory && (
            <div className="mb-4 p-3 bg-gray-100 rounded text-left">
              <p className="text-sm font-medium text-gray-700">Output Location:</p>
              <p className="text-sm font-mono break-all">{outputDirectory}</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-between">
              <button
                ...
                disabled={!file || isProcessing || isPreviewing}
                className={`bg-indigo-500 text-white px-6 py-2 rounded flex items-center ${
                  (!file || isProcessing || isPreviewing) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-indigo-600'
                }`}
              >
                {isPreviewing ? (
                  <>
              ...
              <button
                ...
                className={`bg-blue-500 text-white px-6 py-2 rounded flex items-center ${
                  (!file || isProcessing || isPreviewing) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-600'
                }`}
              >
                {isProcessing ? (
                  <>
            ...
            <div className="mt-8 border-t pt-6">
              ...
              {previewData.error ? (
                ...
                <div className="bg-gray-50 p-4 rounded">
                  <div className="mb-4">
                    <h3 className="text-md font-medium mb-2">Document Info</h3>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        <span className="font-medium">Sheet Name:</span> {previewData.metadata?.sheetName || 'N/A'}
                      </div>
                      <div>
                        <span className="font-medium">Total Rows:</span> {previewData.metadata?.totalRows || 'N/A'}
                      </div>
                      <div>
                        <span className="font-medium">Applied Template:</span> {previewData.config?.templateType || 'default'}
                      </div>
                      <div>
                        <span className="font-medium">Bags:</span> {previewData.config?.bags || 'Auto-detected'}
                      </div>
                    </div>
                  </div>
                      ...
                      <thead>
                        <tr className="bg-gray-200">
                          {previewData.headers?.map((header, idx) => (
                            <th key={idx} className="py-2 px-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider border">
                              {header || `Column ${idx + 1}`}
                            </th>
                          ))}
                        </tr>
                        ...
                        {previewData.rows?.map((row, rowIdx) => (
                          <tr key={rowIdx} className={rowIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                            {row.map((cell, cellIdx) => (
                              <td key={cellIdx} className="py-2 px-3 text-sm border">
                                {cell}
                              </td>
                            ))}
                          </tr>
                  ...
                  {previewData.metadata?.totalRows > previewData.metadata?.previewRows && (
                    <div className="text-center text-sm text-gray-500 mt-2">
                      Showing {previewData.metadata?.previewRows} of {previewData.metadata?.totalRows} rows
                    </div>
                  )}
                </div>
              )}
